//! Tests for `crate::live` — JSON read/parse/write round-trips and
//! the empty/missing-file path.
...
fn mutating_via_data_mut_persists_through_round_trip() {
    ...
    let read_back = Live::from_path(&path).expect("read");
    assert_eq!(read_back.data().pointer("/trailing"), Some(&serde_json::json!(true)));
    assert_eq!(read_back.data().pointer("/editor/tabSize"), Some(&serde_json::json!(2)));
}
